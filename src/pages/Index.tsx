...
const successStories: SuccessStory[] = [
  {
    ...
    quote:
      "銀行との面談で求められる説明が増える中、AI診断のレポートが粗利改善と投資回収の筋道を落ち着いて示してくれました。会議資料づくりに追われず、次の一手を語る時間が持てるようになりました。",
    summary:
      "主要製品ラインの原価高騰で投資判断が揺らいでいた状況を、AIが不採算ラインと改善余地を整理。診断士が資金計画と交渉資料を磨き込み、短期間で粗利が上向き、交渉の場で自信をもって話せる体制が整いました。",
    challenge:
      "原価上昇により粗利率が低迷し、設備投資の優先順位が定まらず、金融機関に示す計画の説得力が不足していました。",
    aiRole:
      "財務・生産データを統合し、製品別の採算とキャッシュへの影響をスコア化。改善の順序を提示しました。",
    expertRole:
      "製造業を多く支援してきた診断士が投資計画と外部向け資料を整理し、経営会議での伝え方を共に設計しました。",
    governance:
      "月次レビューでダッシュボードを共有し、施策の進み具合と投資回収を定例化。面談想定問答も事前に準備しました。",
    metrics: [
      {
        label: "会議準備時間",
        before: "週10時間超",
  ...
  },
  {
    ...
    quote:
      "在庫と広告の数字が散らばっていたのですが、AIが利益に直結するSKUと販促の優先順位を整理。専門家の伴走で店舗とECの会議が数字ベースになり、落ち着いて判断できるようになりました。",
    summary:
      "複数店舗とECを運営する中で在庫ロスと販促費が膨らんでいた状況を、AIが在庫回転・広告効果・顧客動向を統合して整理。診断士が会議資料と計画の型を整え、売上が安定成長の軌道に戻りました。",
    challenge:
      "在庫と広告施策が部門ごとに分断され、粗利と資金が不安定で意思決定に時間がかかっていました。",
    aiRole:
      "販売・在庫・広告データをリアルタイムで連携し、利益貢献度と需要の変化をスコアリングしました。",
    expertRole:
      "流通業を熟知した診断士が販促計画と資金繰り表を整え、会議で語る要点を磨き込みました。",
    governance:
      "週次の商談会でダッシュボードを共有し、施策別の指標と資金繰りを同じ画面で確認する運用を定着させました。",
    metrics: [
      {
        label: "売上高",
        before: "月1.1億円規模",
  ...
  },
  {
    ...
    summary:
      "現場ごとの原価と入金タイミングが見えにくく、資金調達の判断が遅れがちだった状況を、AIが案件別キャッシュフローを即時可視化して解消。専門家が資金繰り表と計画書を金融機関基準で整え、安心して追加資金を確保できる体制を築きました。",
    challenge:
      "工事案件の原価管理が属人化し、資金繰りの山谷が見えず金融機関対応に時間がかかっていました。",
    aiRole:
      "案件別の入出金予測と工数実績をリアルタイムで更新し、リスクの兆しを自動通知しました。",
    expertRole:
      "金融機関の視点を知る専門家チームが交渉資料と想定問答を準備し、経営陣の説明負担を軽減しました。",
    governance:
      "隔週の進捗会議でキャッシュ見通しと工事進捗をセットで共有し、意思決定ログも自動記録しました。",
    metrics: [
      {
        label: "キャッシュ残高",
        before: "月末6,800万円規模",
...
const customerHighlights: CustomerHighlight[] = [
  {
    ...
    alt: "東和精密工業株式会社のロゴ",
    category: "製造×資金繰り",
    proof: "製品別の原価と資金の流れを一枚で整理",
    result: "粗利が右肩上がりに",
    comment:
      "金融機関との面談で迷いなく説明できるようになり、社内でも次の投資判断を気持ちよく決められるようになったとの声。",
    detail: "会議準備の負担を圧縮 / 投資と資金管理を同じ指標で共有 / 対外説明が短時間で完結",
  },
  {
    logo: logoSansan,
    alt: "藤崎商事株式会社のロゴ",
    category: "小売×在庫最適化",
    proof: "在庫と販促データを一体管理し優先順位を明確化",
    result: "売上が安定成長軌道へ",
    comment:
      "現場責任者が自信を持って施策を提案できるようになり、役員会の意思決定が落ち着いて進むようになったとのこと。",
    detail: "在庫ロスを即時に可視化 / 顧客動向の変化をスグ共有 / 施策ごとの採算を色分け",
  },
  {
    logo: logoRaksul,
    alt: "北斗設備工業株式会社のロゴ",
    category: "建設×資金調達",
    proof: "案件ごとの資金繰りと進捗を日次で見える化",
    result: "追加資金を安心して確保",
    comment:
      "現場と本社が同じ画面を見て対話できるため、取引先や金融機関にも落ち着いて説明できるようになったと評価。",
    detail: "キャッシュの先読みを定例化 / 工程と資金の連動をチェック / 交渉資料を素早く更新",
  },
];
...
const representativeProfile: RepresentativeProfile = {
  ...
  title: "代表 / 中小企業診断士",
  summary:
    "福岡を拠点に製造・建設・ITサービス企業の経営改善に携わり続けてきた実務家。AIと専門家のチームで各社専用の管理ダッシュボードを設計し、経営者が取引先や金融機関の前で堂々と語れる計画づくりを支援しています。",
  ...
  achievements: [
    "金融機関と連携した資金戦略の伴走支援を多数実施",
    "生成AIを活用した経営計画づくりと実行支援を継続提供",
    "年商5,000万〜15億円規模の再成長プロジェクトを指揮",
  ],
  affiliations: [
    "一般社団法人Lognowa 経営支援パートナー",
    "地域金融機関・支援機関のアドバイザリーボード",
  ],
};
...
const trustDataPoints: TrustDataPoint[] = [
  {
    ...
    label: "業務効率が向上したと回答",
    description:
      "Workast調査では生成AIを導入した企業の約8割が業務効率の改善を実感しています。",
    source: "Workast (2024)",
    url: "https://workast.com/blog/artificial-intelligence-in-the-workplace/",
    signal: "実証データに基づく生産性向上",
  },
  {
    value: "約9割",
    label: "収益改善を実感",
    description:
      "同調査で約9割の企業が収益改善を報告し、投資判断の裏付けとなっています。",
    source: "Workast (2024)",
    url: "https://workast.com/blog/artificial-intelligence-in-the-workplace/",
    signal: "収益インパクトの裏付け",
  },
  {
    ...
    label: "生産性の底上げ",
    description:
      "Stanford HAIは生成AI支援で平均14%の生産性向上を確認し、確かな効果を提示しています。",
...
const expertCards: ExpertCard[] = [
  {
    name: "田中 圭",
    title: "金融機関出身アドバイザー",
    bio: "法人融資の現場で培った知見をもとに、資金計画と対話準備を支援。",
    photo: expertTanakaPhoto,
    credentials: [
      { icon: ShieldCheck, label: "融資審査のポイントを整理" },
      { icon: LineChart, label: "資金繰りシナリオ設計に強み" },
    ],
  },
  {
    name: "小林 真",
    title: "経営デザインストラテジスト",
    bio: "事業再生と新規事業の立ち上げに伴走し、数値と現場の橋渡しを担う。",
    photo: expertKobayashiPhoto,
    credentials: [
      { icon: BarChart3, label: "中期計画を現場仕様に翻訳" },
      { icon: Workflow, label: "部門横断プロジェクトを推進" },
    ],
  },
  {
    name: "斎藤 美咲",
    title: "財務・管理会計スペシャリスト",
    bio: "成長投資と日々の管理を両立させる仕組みづくりをサポート。",
    photo: expertSaitoPhoto,
    credentials: [
      { icon: Award, label: "管理会計の設計と定着支援" },
      { icon: FileText, label: "資金計画のレビュー体制を整備" },
    ],
  },
];
...
const resourceCards: ResourceCard[] = [
  ...
  },
  {
    ...
    description:
      "生成AIを活かした中堅企業の最新事例をまとめ、投資対効果の考え方とリスク対策を整理しました。",
    highlights: [
      "導入前後の指標推移と投資対効果の整理",
      "社内浸透を進めた工夫とつまずき例",
      "セキュリティと運用ルールの確認ポイント",
          ...
          <div className="container">
            ...
            <div className="resource-subsection" data-animate>
              <div className="resource-subsection__header">
                ...
                <p>
                  最新の成功事例や資金計画の勘どころをまとめたオンライン／オフラインイベントを定期開催。
